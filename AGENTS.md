...
- `src/lib` is grouped by domain: `map/` for MapLibre configuration and URL view state, `geo/` for geometry and spatial indexing, `import/` for file discovery and parsers, `tracks/` for track-derived data, `ui/` for presentation helpers and preferences, and `storage/` for IndexedDB persistence.
- The map is implemented with MapLibre GL in globe projection using raster base maps. Base map definitions live in `src/lib/map/basemaps.ts`; the MapLibre style assembly lives in `src/lib/map/style.ts`. The OSM dark basemap uses CARTO Dark Matter raster tiles, which are based on OpenStreetMap data and require both OpenStreetMap and CARTO attribution.
...
- Current map position, zoom, bearing, and pitch are synchronized with the query string via `src/lib/map/urlState.ts`; use `history.replaceState` so map movement does not pollute browser history.
- Theme mode support lives in `src/lib/ui/theme.ts` and is controlled from the right-side toolbar. Supported modes are `system`, `light`, and `dark`; the preference is persisted in localStorage, while `system` follows `prefers-color-scheme`.
...
- UI language preference is persisted in localStorage. Parsed track workspace snapshots are persisted in IndexedDB via `src/lib/storage/workspaceStore.ts` so accidental reloads or development remounts do not discard uploaded route data; this still stays entirely in the browser.
- GPX and FIT inputs are normalized into the shared `Track` / `TrackPoint` model in `src/types.ts`.
- GPX parsing lives in `src/lib/import/parsers.ts` via `fast-xml-parser`.
...
- Drag/drop import, recursive folder traversal, and supported extension filtering live in `src/lib/import/fileDrop.ts`. Supported file extensions include `.gpx`, `.fit`, `.gpx.gz`, and `.fit.gz`.
...
- Spatial filtering and geographical range queries use a geohash candidate pass in `src/lib/geo/geohash.ts`, then exact route/selection intersection checks in `src/lib/geo/geometry.ts`. Do not rely on geohash alone for correctness.
